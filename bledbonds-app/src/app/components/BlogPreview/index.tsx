...
import getBlogData from "@/app/api/getBlogData";
    ...
    <div className="grid grid-cols-3 gap-3">
      {blogData.map((post, index) => (
        <article key={index} className="p-6 bg-palette-11 rounded-lg border border-palette-9 shadow-md">
          ...
          <h2 className="mb-2 text-2xl font-bold tracking-tight text-palette-1"><a href="/">{post.title}</a></h2>
          <p className="mb-5 font-light text-palette-1">{post.resume}</p>
          <div className="flex justify-between items-center">
            <div className="flex items-center space-x-4">
              <img className="w-7 h-7 rounded-full" src={post.postByAvatar} alt="Jese Leos avatar" />
